...
CREATE TABLE `activity_logs` (
  ...
  `log_short_description` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL,
  `log_full_description` text COLLATE utf8mb4_unicode_ci NOT NULL,
...
INSERT INTO `messages` VALUES (1,2,77,'tdfgu,uymjtnhrgbfvdv',NULL,'2026-05-05 00:46:10','2026-05-05 00:46:10');
...
CREATE TABLE `po_items_tbl` (
  ...
  `po_items_category` enum('supply_and_materials','semi-expendable','equipment') DEFAULT NULL,
...
INSERT INTO `po_items_tbl` VALUES (57,1,NULL,1,'Liter','Coke Zero 1.5',1,85.00,NULL,85.00,'supply_and_materials'),(58,1,NULL,1,'Box','Pancit Canton',3,20.00,NULL,60.00,'supply_and_materials'),(59,1,NULL,1,'Meter','12312',12,23.00,NULL,276.00,'supply_and_materials'),(60,1,NULL,1,'Piece','DSLR Camera',2,50000.00,NULL,100000.00,'semi-expendable'),(61,1,NULL,1,'Piece','Anthropic Enterprise Claude Code Subscription',2,50000.00,NULL,100000.00,'equipment');
...
CREATE TABLE `pr_items_tbl` (
  ...
  `pr_items_category` enum('supply_and_materials','semi-expendable','equipment') DEFAULT NULL,
...
INSERT INTO `pr_items_tbl` (`pr_items_id`, `pr_id_fk`, `pr_app_item_id_fk`, `pr_items_quantity`, `pr_items_unit`, `pr_items_cost`, `pr_items_descrip`, `bidding_status`, `pr_items_category`) VALUES (1,5,6,1,'Set',50000.00,'Test2','pending','equipment'),(2,5,7,1,'Piece',599.00,'IVOS Summer blast Ticket','pending','supply_and_materials'),(3,6,4,1,'Piece',40000.00,'Laptop','pending','semi-expendable'),(10,7,6,1,'Lot',50000.00,'Animation Machine','pending','equipment'),(11,7,6,1,'Set',500.00,'Photo Paper','pending','supply_and_materials'),(12,7,6,1,'Piece',5000.00,'Animation Lightbox','pending','semi-expendable'),(13,7,7,1,'Piece',500.00,'IVOS Summer blast Ticket','pending','supply_and_materials'),(69,8,8,10,'Piece',3000.00,'Upuan','pending','semi-expendable'),(70,8,8,5,'Piece',5000.00,'lamesa','pending','semi-expendable'),(71,8,9,2,'Piece',40000.00,'Laptop','pending','equipment'),(72,9,5,1,'Piece',550.00,'IVOS Ticket','pending','supply_and_materials');
...
CREATE TABLE `ris_items_tbl` (
  `ris_item_id` bigint unsigned NOT NULL AUTO_INCREMENT,
  `ris_id_fk` bigint unsigned NOT NULL,
  `ris_po_items_id_fk` bigint unsigned NOT NULL,
  ...
  `ris_remarks` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`ris_item_id`),
  KEY `fk_ris_items_header` (`ris_id_fk`),
  KEY `fk_ris_items_po` (`ris_po_items_id_fk`),
  CONSTRAINT `fk_ris_items_header` FOREIGN KEY (`ris_id_fk`) REFERENCES `ris_tbl` (`ris_id`) ON DELETE CASCADE,
  CONSTRAINT `fk_ris_items_po` FOREIGN KEY (`ris_po_items_id_fk`) REFERENCES `po_items_tbl` (`po_items_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `ris_tbl` (
  ...
  `po_id_fk` bigint unsigned NOT NULL,
  `ris_fund_cluster` varchar(50) DEFAULT NULL,
  `ris_division` varchar(100) DEFAULT NULL,
  `ris_office` varchar(100) DEFAULT NULL,
  ...
  `ris_requested_date` varchar(50) DEFAULT NULL,
  `ris_approved_by` bigint unsigned DEFAULT NULL,
  `ris_approved_designation` varchar(100) DEFAULT NULL,
  `ris_approved_date` varchar(50) DEFAULT NULL,
  `ris_issued_by` bigint unsigned DEFAULT NULL,
  `ris_issued_designation` varchar(100) DEFAULT NULL,
  `ris_issued_date` varchar(50) DEFAULT NULL,
  `ris_received_by` bigint unsigned DEFAULT NULL,
  `ris_received_designation` varchar(100) DEFAULT NULL,
  `ris_received_date` varchar(50) DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`ris_id`),
  KEY `fk_ris_po` (`po_id_fk`),
  KEY `fk_ris_requested` (`ris_requested_by`),
  KEY `fk_ris_approved` (`ris_approved_by`),
  KEY `fk_ris_issued` (`ris_issued_by`),
  KEY `fk_ris_received` (`ris_received_by`),
  CONSTRAINT `fk_ris_approved` FOREIGN KEY (`ris_approved_by`) REFERENCES `users` (`user_id`),
  CONSTRAINT `fk_ris_issued` FOREIGN KEY (`ris_issued_by`) REFERENCES `users` (`user_id`),
  CONSTRAINT `fk_ris_po` FOREIGN KEY (`po_id_fk`) REFERENCES `po_tbl` (`po_id`) ON DELETE CASCADE,
  CONSTRAINT `fk_ris_received` FOREIGN KEY (`ris_received_by`) REFERENCES `users` (`user_id`),
  CONSTRAINT `fk_ris_requested` FOREIGN KEY (`ris_requested_by`) REFERENCES `users` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `sessions` VALUES ('cfkPeU6IkSnJFUaKjeQwqypCpu6aOO4wcKhCfktv',78,'127.0.0.1','Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Safari/537.36 Edg/148.0.0.0','',1778687985),('I89rvR9Fum6gmIdEB09TKdReFHxU0mPZ6M01yjmp',78,'127.0.0.1','Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Safari/537.36','',1778687585),('J4B3mrBiILK5Dcr2QIFsDeJ47kJXpZpBbICDaM2f',NULL,'127.0.0.1','Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Safari/537.36','',1778748215),('WpAuTqDS29uWJrlCI8GBb2sL8QSGmHyy8TcNPOkB',5,'127.0.0.1','Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Safari/537.36 Edg/148.0.0.0','',1778688057);
...
INSERT INTO `user_departments_tbl` VALUES (1,5,1),(2,74,1),(5,77,36),(6,78,15),(7,19,35);
...
INSERT INTO `user_roles_tbl` VALUES (1,2,1),(4,59,5),(6,77,6),(7,5,11),(8,78,10);
...
-- Dump completed on 2026-05-14 16:49:27
