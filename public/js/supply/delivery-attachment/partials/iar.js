...
$(document).ready(function() {
    ...
    $(document).on('click', '.iar-container .add-item-btn', function(e) {
        ...
        var firstRow = tbody.find('tr.iar-item-row').first();
    ...
    $(document).on('click', '.iar-container .remove-row-btn', function(e) {
        ...
        if (tbody.find('tr.iar-item-row').length <= 1) {
            Swal.fire({
                icon: 'warning',
                title: 'Action Denied',
    ...
    function convertLaravelKeyToInputName(key) {
        // Convert e.g., "items.0.iar_quantity" to "items[0][iar_quantity]"
        if (key.includes('.')) {
            var parts = key.split('.');
            var name = parts[0];
    ...
    function clearIarErrors(form) {
        form.find('.is-invalid').removeClass('is-invalid');
        form.find('.field-error').text('').addClass('d-none');
    }

    function showIarErrors(form, errors) {
    ...
    $(document).on('submit', '.iar-container form', function(e) {
        e.preventDefault();
        var form = $(this);
        ...
        .then(function(result) {
            ...
            if (result.status === 422 && result.data.errors) {
                ...
                showIarErrors(form, result.data.errors);
