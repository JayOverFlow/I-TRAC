...
$(document).ready(function() {
    ...
    function convertLaravelKeyToInputName(key) {
        // Convert e.g., "items.0.par_quantity" to "items[0][par_quantity]"
        if (key.includes('.')) {
            var parts = key.split('.');
            var name = parts[0];
    ...
    function clearParErrors(form) {
        form.find('.is-invalid').removeClass('is-invalid');
        form.find('.field-error').text('').addClass('d-none');
    }

    function showParErrors(form, errors) {
    ...
    $(document).on('submit', '.par-container form', function(e) {
        e.preventDefault();
        var form = $(this);
        ...
        .then(function(result) {
            ...
            if (result.status === 422 && result.data.errors) {
                ...
                showParErrors(form, result.data.errors);
