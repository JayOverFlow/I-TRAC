document.addEventListener('DOMContentLoaded', function () {
    ...
    if (formValidation) {
        ...
        // Auto-capitalize TUPT-ID and auto-insert hyphens as user types with high-fidelity static mask
        const tupIdField = document.querySelector('#tup-id');
        if (tupIdField) {
            function updateMask() {
                let val = tupIdField.value;
                ...
                tupIdField.value = formatted;
                
                let firstUnderscore = formatted.indexOf('_');
                let cursorPosition = firstUnderscore !== -1 ? firstUnderscore : 13;
                
                setTimeout(() => {
                    tupIdField.setSelectionRange(cursorPosition, cursorPosition);
                }, 0);
            }

            tupIdField.addEventListener('focus', function() {
                if (!tupIdField.value || tupIdField.value === '') {
                    tupIdField.value = '_____-__-____';
                }
                updateMask();
            });

            tupIdField.addEventListener('click', updateMask);

            tupIdField.addEventListener('keydown', function(e) {
                if (e.key === 'Backspace') {
                    e.preventDefault();
                    let raw = tupIdField.value.replace(/[-_]/g, '');
                    if (raw.length > 0) {
                        let newRaw = raw.substring(0, raw.length - 1);
                        tupIdField.value = newRaw;
                        updateMask();
                    }
                    return;
                }

                const allowedKeys = ['Delete', 'ArrowLeft', 'ArrowRight', 'Tab', 'Enter'];
            ...
            tupIdField.addEventListener('input', updateMask);

            tupIdField.addEventListener('blur', function() {
                if (tupIdField.value === '_____-__-____') {
                    tupIdField.value = '';
                }
            });
        }

        // Auto-restrict Contact Number to numerical digits and max length of 11
        const contactNoField = document.querySelector('#contact-no');
        if (contactNoField) {
        ...
        function validateStepOne() {
            ...
            const stepOneValidation = {
                ...
                tupID: {
                    el: tupID,
                    required: true,
                    pattern: /^[A-Z0-9]{5}-\d{2}-\d{4}$/i,
                    messages: {
                        required: 'TUPT-ID is required.',
                        pattern: 'TUPT-ID must follow the format XXXXX-00-0000.'
                    }
                }
            };
            ...
            const stepTwoValidation = {
                ...
                contactNo: {
                    ...
                    messages: {
                        ...
                        pattern: 'Contact number must be exactly 11 digits starting with 09.'
                    }
                },
                password: {
                    el: password,
                    ...
                    if (result.success) {
                        ...
                        if (result.errors) {
                            for (let field in result.errors) {
                                ...
                                if (field === 'contact_no') setFieldInvalid(contactNo, result.errors[field][0]);
   ...
   // Collect user inputs for review (step 3)
        function collectFormData() {
            return {
                ...
                contactNo: document.querySelector('#contact-no').value,
        ...
        function updateReviewStep() {
            ...
            document.querySelector('#review-contact-no').textContent = data.contactNo;
            document.querySelector('#review-password').textContent = data.password;
            
            // Update right column  
            document.querySelector('#review-middle-name').textContent = data.middleName || 'N/A';
        ...
        // OTP Input Validation and Auto-Focus Management
        document.querySelectorAll('.opt-input').forEach((input, index) => {
            ...
            input.addEventListener('input', function(e) {
                ...
                if (this.value.length === 1 && index < 5) {
                    document.querySelectorAll('.opt-input')[index + 1].focus();
                }
            });
            ...
            input.addEventListener('paste', function(e) {
                ...
                pastedData.split('').forEach((digit, i) => {
                    if (i < 6) {
                        document.querySelectorAll('.opt-input')[i].value = digit;
                    }
                });
                
                // Focus last filled input or next empty one
                const lastFilledIndex = Math.min(pastedData.length, 5);
                document.querySelectorAll('.opt-input')[lastFilledIndex].focus();
            });
        ...
        document.querySelector('.btn-submit').addEventListener('click', function() {
            ...
            // Validate each input and build code
            ...
            if (!isValid || code.length !== 6) {
                showModernAlert('Please enter a valid 6-digit verification code.', 'error');
                inputs[0].focus();
                return;
            }
            ...
            .then(data => {
                if (data.success) {
                    showModernAlert('Email verified successfully!', 'success');
                    ...
                    .then(registrationData => {
                        if (registrationData.success) {
                            showModernAlert('Registration completed successfully! Redirecting to login...', 'success');
                            setTimeout(() => {
                                window.location.href = '/login';
                            }, 2000);
                        } else {
                            showModernAlert('Registration failed: ' + registrationData.message, 'error');
                    ...
                    .catch(error => {
                        ...
                        showModernAlert('Error completing registration. Please try again.', 'error');
                        // Re-enable verify button
                        this.disabled = false;
                        this.textContent = 'VERIFY';
                    });
                    ...
                    this.disabled = false;
                    this.textContent = 'VERIFY';
                    // Clear inputs and show validation
                    inputs.forEach(input => {
                        input.value = '';
                        input.classList.add('is-invalid');
                    });
