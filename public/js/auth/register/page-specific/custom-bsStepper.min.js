document.addEventListener('DOMContentLoaded', function () {
    ...
    if (formValidation) {
        ...
        function validateStepTwo() {
            ...
            const userTypeContainer = currentStep?.querySelector('.col-6');
            ...
            toggleFeedbackInContainer(userTypeContainer, false);
            ...
            if (!userType) {
                toggleFeedbackInContainer(userTypeContainer, true);
                if (userTypeContainer) {
                    const feedbackEl = userTypeContainer.querySelector('.invalid-feedback');
        ...
        //     // Proceed to step 4
        //     const stepper = new bsStepper(document.querySelector('.bs-stepper'));
        //     stepper.next();
        ...
        document.querySelector('#send-code-btn').addEventListener('click', function() {
            ...
            // Disable the Send Code button to avoid spam
            this.disabled = true;
            // Feedback to user
            ...
            fetch('/email/send-code', {
                method: 'POST',
                headers: {
                    // This means we are sending JSON string to popolate $request object for us to access
                    // It tells the server to expect data formatted like {"key": "value"}
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content') // For security
                },
                body: JSON.stringify({email: email}) // Converting the JavaScipt object email to string text (JSON)
            })
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    const modalElement = document.querySelector('#exampleModalCenter');
                    const modal = bootstrap.Modal.getInstance(modalElement);
                    if (modal) {
                        modal.hide();
                        // Force remove backdrop if it doesn't disappear automatically
                        setTimeout(() => {
